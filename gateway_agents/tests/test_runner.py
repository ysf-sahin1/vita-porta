...
def test_run_once_returns_bundle_with_three_observations(
    ...
    assert "gait" in payload
    assert "skin" in payload
    assert "respiration" in payload
    assert payload["gait"]["agent"] == "gait"
    assert payload["skin"]["agent"] == "skin"
    assert payload["respiration"]["agent"] == "respiration"
